...
--TELEPORT TO RANDOM
--creates a new rift and teleports the player
local teleport = function(player, pos, x, y, z)
		...
		minetest.get_voxel_manip():read_from_map(riftpos, riftpos)
		if not minetest.get_node_or_nil(riftpos) then
			minetest.emerge_area(vector.subtract(riftpos, 10), vector.add(riftpos, 10))
		end
...
--RIFT RETURN
--player is transported via linked rifts with risk of explosion
local rift_return = function(player, pos)
	...
	if tx ~= nil and ty ~=nil and tz ~= nil then
		...
		local blow_chance = math.random(1, risk)
		...
		if blow_chance <= 1 then
			-- force emerge of target area
			minetest.get_voxel_manip():read_from_map(target_coords, target_coords)
			if not minetest.get_node_or_nil(target_coords) then
				minetest.emerge_area(vector.subtract(target_coords, 10), vector.add(target_coords, 10))
			end
			--remove portal
		...
		if blow_chance <= 1 then
			...
			if not minetest.get_node_or_nil(pos) then
				minetest.emerge_area(vector.subtract(pos, 10), vector.add(pos, 10))
			end
			--remove portal
...
minetest.register_node('luckyportal:rift', {
	...
	on_punch = function(pos, node, puncher)
		...
		if math.random(1, risk) > 1 then
			--most likely...send them to where they expect
			rift_return(puncher, pos)
		else
			--least likely,... create a new linked wormhole pair
			teleport(puncher, pos, 30000, 30000, 30000)
		end
	end,
	on_blast = function(pos)
...
--[[
--disabled these "walk-in" actions because have disabled walkablility of nodes.
--disabled walkabliity so they don't get destroyed by having stuff fall on them
...
minetest.register_abm{
  ...
  chance = 2,
  action = function(pos, node, active_object_count, active_object_count_wider)
		minetest.sound_play("luckyportal_noise", {pos = pos, gain = 5, max_hear_distance = 20,})
		minetest.sound_play("luckyportal_thunder", {pos = pos, gain = 7, max_hear_distance = 50,})
		flash2(pos)
end,
}
...
minetest.register_abm{
  ...
  action = function(pos, node, active_object_count, active_object_count_wider)
		...
		minetest.sound_play("luckyportal_thunder", {pos = pos, gain = 20, max_hear_distance = 200,})
		minetest.sound_play("luckyportal", {pos = pos, gain = 4.0, max_hear_distance = 100,})
		minetest.set_node(pos, {name = "air"})
		tnt.boom(pos, {
						radius = 6,
						damage_radius = 6,
